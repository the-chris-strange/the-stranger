...
describe('vitest-config generator package dependencies utility', () => {
  ...
  beforeAll(async () => {
    ...
    spy = vi.spyOn(await import('../../lib/add-dependencies.js'), 'addDependenciesToProject')
  })
  ...
  it('adds vitest dependencies', () => {
    addDependencies(tree, { project: 'test' }, pkg)

    expect(spy).toHaveBeenCalledExactlyOnceWith(
      tree,
      [],
      expect.arrayContaining(['nx', '@nx/vitest', 'vitest', '@vitest/coverage-v8']),
      pkg,
    )
  })
})
