...
describe('vite config generator', () => {
  ...
  it('creates a config by default', async () => {
    ...
    expect(tree.exists('packages/test/vite.config.mts')).toBe(true)
  })
  ...
  it('sets import path for react plugin if swc is not set', async () => {
    ...
    options.swc = false
    await viteConfigGenerator(tree, options)
    expect(tree.read('packages/test/vite.config.mts', 'utf8')).toContain(
  ...
  it("doesn't set `target` if no value is provided", async () => {
    ...
    await viteConfigGenerator(tree, options)
    expect(tree.read('packages/test/vite.config.mts', 'utf8')).not.toContain(
      'target: [',
    )
  })
  ...
  it("doesn't set `rollupExternals` if no value is provided", async () => {
    ...
    await viteConfigGenerator(tree, options)
    expect(tree.read('packages/test/vite.config.mts', 'utf8')).not.toMatch(
  ...
  it('renders `rollupExternals` correctly if a value is provided', async () => {
    ...
    await viteConfigGenerator(tree, options)
    expect(tree.read('packages/test/vite.config.mts', 'utf8')).toMatch(
