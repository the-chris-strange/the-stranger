...
import { getPropertyAtPath, setPropertyPath, splitPropertyPath } from './property-path'

describe('propertyPath', () => {
  it('gets top-level properties', () => {
    const obj = { foo: 'bar' }
    const path = 'foo'
    expect(getPropertyAtPath(obj, path)).toBe('bar')
  })
  ...
  it('gets nested properties', () => {
    ...
    expect(getPropertyAtPath(obj, path)).toBe(42)
  })
})
...
describe('setPropertyPath', () => {
  it('creates nested objects via dot path', () => {
    ...
    expect(getPropertyAtPath(obj, 'foo.bar.baz')).toBe(7)
  })
  ...
  it('creates nested arrays via bracket path', () => {
    ...
    expect(getPropertyAtPath(obj, 'items[1].name')).toBe('thing')
  })
  ...
  it('sets values inside Maps', () => {
    ...
    expect(obj.get('level1')).toStrictEqual(['ok'])
    expect(getPropertyAtPath(obj, 'level1[0]')).toBe('ok')
  })
})
